...
def load_cfd_data(folder):
    file_path = os.path.join('', folder, 'r_0_5_U.xy')
...
def plot_Ux_r05(outDir,expData):
    folders=[outDir]
    # load EXP data as .dat file
    file_path2 = expData+'/AxIn_NewMexico.dat'
    ...
    axs[0].set_title('Ux vs. x, r=0.5', fontsize=20)
    ...
    axs[1].set_title('Uy vs. x, r=0.5', fontsize=20)
    ...
    axs[2].set_title('Uz vs. x, r=0.5', fontsize=20)
    ...
    # Save the figure
    if save_plots:
        plt.savefig(outDir+'/U_x_r05.png', dpi=300, bbox_inches='tight')
