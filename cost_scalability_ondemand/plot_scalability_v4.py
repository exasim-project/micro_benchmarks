...
GPUHr = 2.74470525   # Dollar per GPU-hour
CoreHr = 0.086953125   # Dollar per CPU-core-hour
# ----------------------------------------------------------------------------------------
...
#    GPUHr = 1.17175   # Euro per GPU-hour
#    CoreHr = 0.03742   # Euro per CPU-core-hour
    ...
    for element in data_list:
        ...
        costGPU = ((GPUHr/3600) * (times/iter_per_dt) * (4 * nodes))/nCells
        costCPU = ((CoreHr/3600) * (times/iter_per_dt) * (76 * nodes))/nCells
...
def main():
    ...
    if args.spuma_json:
        ...
        for lst in spuma_raw.values():
            for ent in lst:
                ...
                spuma_dict.setdefault(solver, {'nodes':[], 'procs': [],'cost': [],'fvops':[],'times':[]})
                spuma_dict[solver]['nodes'].append(ns)
                spuma_dict[solver]['procs'].append(ng)
                spuma_dict[solver]['cost'].append((GPUHr/3600)/fv)
        ...
        for sol,dat in spuma_dict.items():
            ...
            spuma_dict[sol]['cost'] = np.array(dat['cost'])[idx]
    ...
    for sol in solver_order:
        ...
        p = np.array(data_dict[sol]['procs']); f = np.array(data_dict[sol]['cost'])
        if sol == 'GAMG' or sol == 'PCG':
            if peak_ref < max(data_dict[sol]['fvops']):
                peak_ref = max(data_dict[sol]['fvops'])
        ax.plot(p, 1e12*f, marker='o', linestyle='-', label=label_map[sol], zorder=2)
    # SPuMA overlay
    for sol in ('PCG','GAMG'):
        if sol in spuma_dict:
            procs_sp = spuma_dict[sol]['procs']; fv_sp = spuma_dict[sol]['cost']
            clr = ucfdGrey if sol=='PCG' else ucfdBlue
            ax.plot(procs_sp, 1e12*fv_sp, marker='s', linestyle=':', linewidth=2, color=clr,
                    label=f"SPUMA: GPU ({sol} solver)")
    if args.spuma_json:
        ax.plot(12, 1e12*((GPUHr/3600)/35170250.9), marker='s',color='r', linestyle='none', label="Comm. ref.")
    #ax.set_yscale('log')
    if args.y_limits: ax.set_ylim(args.y_limits)
    ax.set_ylabel('Cost/TFVO [$\$$]')
