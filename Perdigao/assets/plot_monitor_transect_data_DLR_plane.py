...
#df_turbine = pd.DataFrame(data=turbine, columns = ['x', 'y', 'z'])
#df_turbine["coord"] = np.sqrt(df_turbine['x'].mul(df_turbine['x']) + df_turbine['y'].mul(df_turbine['y']))

df_profil = pd.read_csv("../00_caseFiles/assets/Hoehenprofil_DLR.csv", sep = ';')
df_profil["coord"] = np.sqrt(df_profil['x'].mul(df_profil['x']) + df_profil['y'].mul(df_profil['y']))

df_WS1 = pd.read_csv("../00_caseFiles/assets/DLR85_170510_1530.txt", sep=',', header = None)
df_WS1[7] = -df_WS1[7]
df_all = df_WS1
...
reader.SetFileName(latestTime+"/DLR_plane.vtp")
reader.Update()
...
ax.set_title('DLR Lidar data, Processing: N. Wildmann (DLR)',fontsize=10)
...
#ax.scatter(df_turbine['coord'], df_turbine['z'], c='k', s=2)
...
fig.savefig("fig_monitor_DLR_plane.png", bbox_inches='tight',dpi=600)