...
def prepareData_final(data_list, ndt, nPISOCorr):
    ...
    for element in data_list:
        ...
        fvops = (nCells * iter_per_dt) / times if times > 0 else 0.0
...
def main():
    ...
    if args.spuma_json:
        ...
        for lst in spuma_raw.values():
            for ent in lst:
                ...
                ns = ng/4; fv = (nCells*args.nPISOCorr*iPdt)/dt if dt>0 else 0
    ...
    if args.spuma_json and "Windsor" in case_name:
        ax.plot(12, 422043010.8, marker='s',color='r', linestyle='none', label="Comm. ref.")
    ax.set_yscale('log')
    if args.y_limits: ax.set_ylim(args.y_limits)
    ax.set_ylabel('FVOPS [$s^{-1}$]')
